...
which ingest the work of all the above Oracles to determine the appropriate emissions for each
subDAO.
...
passively receiving tokens, Hotspot Owners will have to _redeem_ them.

When a Hotspot Owner makes a redemption request, the Rewards Oracle will compute the difference
between the `lifetime` and `claimed` token Rewards to determine the number of unclaimed tokens per
...
Oracle will issue HNT Reward emissions to each subDAO's treasury using each subDAO's Utility Score to
...
This is a "one-way" transaction to convert your subDNT into HNT, as you cannot burn HNT for subDNT.

## Other Notes

- Initially, all these Oracles will be run by the Helium Foundation, with the long-term goal that
